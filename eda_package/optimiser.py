...
class OverbookingOptimizer:
    ...
    def compute_showup_distribution(
        ...
        *n_total* bookings.
        ...
        -------
        dict with keys:
            pmf              – list[float], P(show-ups == k) for k in 0..n_total
            mean_showups     – float
            std_showups      – float
            n_total          – int   (echo back)
            n_current        – int   (number of real bookings)
            capacity         – int   (echo back)
            relocation_probability – float, P(show-ups > capacity)
            individual_show_probs  – list[float], per-booking show-up probs used
        """
        ...
        # build the cancel-prob vector for the requested total
